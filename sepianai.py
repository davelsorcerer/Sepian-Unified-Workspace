...
class VisualHead:
    def __init__(self, canvas):
        ...
        try:
            ...
            gaze_path = os.path.join(BASE_DIR, "lookingleft.png")
            if os.path.exists(gaze_path):
                left_img = Image.open(gaze_path).convert("RGBA")
                self.gaze_images["left"] = ImageTk.PhotoImage(left_img)
                center_path = os.path.join(BASE_DIR, "middlelook.png")
                if os.path.exists(center_path):
                    self.gaze_images["center"] = ImageTk.PhotoImage(
                        Image.open(center_path).convert("RGBA")
                    )
                right_path = os.path.join(BASE_DIR, "lookingright.png")
                if os.path.exists(right_path):
                    right_img = Image.open(right_path).convert("RGBA")
                else:
                    right_img = left_img.transpose(Image.Transpose.FLIP_LEFT_RIGHT)
                self.gaze_images["right"] = ImageTk.PhotoImage(right_img)
    ...
    def set_gaze(self, gaze):
        """Set the eye layer to center, left, or right."""
